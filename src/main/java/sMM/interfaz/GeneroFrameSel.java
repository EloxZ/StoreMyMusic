 ...
 * @author Eloy
 */
public class GeneroFrameSel extends javax.swing.JFrame {
 // Variables para mover ventana desde la barra de titulo (Dragger)
    ...
    private HashMap<Integer,Integer> tuplaFilaGenero;
    ...
    public void cargarDatos() {
        ...
        if (du != null && du.getGeneros() != null) {
            int i = 0;
            for (Map.Entry<Integer,Genero> ed : du.getGeneros().entrySet()) {
                String[] listaAtributos = new String[1];
                listaAtributos[0] = ed.getValue().getNombre();
                tuplaFilaGenero.put(i, ed.getValue().getIdentificador());
    ...
    public GeneroFrameSel(MainFrame f, AddDiscoFrame f2) {
        mFrame = f;
        addFrame = f2;
        initComponents();
        configComponentes();
    ...
    public GeneroFrameSel(MainFrame f, ModDiscoFrame f2) {
        mFrame = f;
        modFrame = f2;
        initComponents();
        configComponentes();
    ...
    private void initComponents() {
        ...
        generoLabel.setForeground(new java.awt.Color(254, 254, 254));
        ...
        BaseLayout.setHorizontalGroup(
            ...
            .addGroup(javax.swing.GroupLayout.Alignment.TRAILING, BaseLayout.createSequentialGroup()
                ...
                .addGroup(BaseLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING, false)
                    ...
                    .addComponent(generoField, javax.swing.GroupLayout.PREFERRED_SIZE, 222, javax.swing.GroupLayout.PREFERRED_SIZE)
    ...
    private void subirGenero(String n) {
        ConexionBD bd = ConexionJDBC.getInstance();
        try {
            int id = bd.añadirGenero(new Genero(n));
            mFrame.consultarDiscos();
            //du.addGenero(new Genero(id,n,nac));
            cargarDatos();
    ...
    private void modificarGenero(int id, String n) {
        ConexionBD bd = ConexionJDBC.getInstance();
        try {
            bd.modificarGenero(id, n);
            mFrame.consultarDiscos();
            //du.modificarGenero(id, n, nac);
            cargarDatos();
    ...
    private void aplicarButtonMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_aplicarButtonMouseClicked
        if (evt.getButton() == 1) {
            if (generoField.isEditable() && du != null && !generoField.getText().isEmpty()) { // Nuevo/modificacion disponible
                if (Lista.getSelectedRow() == -1 && !du.existeGenero(generoField.getText())) { // Nuevo Genero
                    subirGenero(generoField.getText());
                } else if (Lista.getSelectedRow() != -1 && !du.existeGeneroIgnorando(generoField.getText(), tuplaFilaGenero.get(Lista.convertRowIndexToModel(Lista.getSelectedRow())))) { // Modificar Genero
                    modificarGenero(tuplaFilaGenero.get(Lista.convertRowIndexToModel(Lista.getSelectedRow())),generoField.getText()); 
                }
                generoField.setEditable(false);
                generoField.setText("");
                Lista.clearSelection();
    ...
    private void eliminarButtonMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_eliminarButtonMouseClicked
        if (evt.getButton() == 1) {
            if (Lista.getSelectedRow() != -1) {
                if (addFrame != null) {
                    addFrame.quitarGenero(tuplaFilaGenero.get(Lista.convertRowIndexToModel(Lista.getSelectedRow())));
                } else {
                    modFrame.quitarGenero(tuplaFilaGenero.get(Lista.convertRowIndexToModel(Lista.getSelectedRow())));
                }
                
                eliminarGenero(tuplaFilaGenero.get(Lista.convertRowIndexToModel(Lista.getSelectedRow())));
            }
        }
    ...
    private void selButtonMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_selButtonMouseClicked
       if (evt.getButton() == 1) {
            if (Lista.getSelectedRow() != -1) {
                if (addFrame != null) {
                    addFrame.seleccionarGenero(tuplaFilaGenero.get(Lista.convertRowIndexToModel(Lista.getSelectedRow())));
                } else {
                    modFrame.seleccionarGenero(tuplaFilaGenero.get(Lista.convertRowIndexToModel(Lista.getSelectedRow())));
                }
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(GeneroFrameSel.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
               // new GeneroFrameSel(null,null).setVisible(true);
            }
        });
    }
