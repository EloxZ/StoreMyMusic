 ...
 * @author Eloy
 */
public class DiscograficasFrameSel extends javax.swing.JFrame {
 // Variables para mover ventana desde la barra de titulo (Dragger)
    ...
    private HashMap<Integer,Integer> tuplaFilaDiscografica;
    ...
    public void cargarDatos() {
        ...
        tuplaFilaDiscografica = new HashMap<>();
        ...
        if (du != null && du.getDiscograficas() != null) {
            int i = 0;
            for (Map.Entry<Integer,Discografica> ed : du.getDiscograficas().entrySet()) {
                String[] listaAtributos = new String[1];
                listaAtributos[0] = ed.getValue().getNombre();
                tuplaFilaDiscografica.put(i, ed.getValue().getIdentificador());
...
//                    String nacionalidad = (String) Lista.getModel().getValueAt(row, 1);
    ...
    public DiscograficasFrameSel(MainFrame f, AddDiscoFrame f2) {
        mFrame = f;
        addFrame = f2;
        initComponents();
        configComponentes();
    ...
    public DiscograficasFrameSel(MainFrame f, ModDiscoFrame f2) {
        mFrame = f;
        modFrame = f2;
        initComponents();
        configComponentes();
    ...
    private void subirDiscografica(String n) {
        ConexionBD bd = ConexionJDBC.getInstance();
        try {
            int id = bd.añadirDiscografica(new Discografica(n));
    ...
    private void modificarDiscografica(int id, String n) {
        ConexionBD bd = ConexionJDBC.getInstance();
        try {
            bd.modificarDiscografica(id, n);
            mFrame.consultarDiscos();
    ...
    private void aplicarButtonMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_aplicarButtonMouseClicked
        if (evt.getButton() == 1) {
            if (nombreField.isEditable() && du != null && !nombreField.getText().isEmpty()) { // Nuevo/modificacion disponible
                if (Lista.getSelectedRow() == -1 && !du.existeDiscografica(nombreField.getText())) { // Nuevo autor
                    subirDiscografica(nombreField.getText());
                } else if (Lista.getSelectedRow() != -1 && !du.existeDiscograficaIgnorando(nombreField.getText(), tuplaFilaDiscografica.get(Lista.convertRowIndexToModel(Lista.getSelectedRow())))) { // Modificar autor
                    modificarDiscografica(tuplaFilaDiscografica.get(Lista.convertRowIndexToModel(Lista.getSelectedRow())),nombreField.getText()); 
                }
                nombreField.setEditable(false);
                nombreField.setText("");
                Lista.clearSelection();
    ...
    private void eliminarDiscografica(int id) {
        ConexionBD bd = ConexionJDBC.getInstance();
        try {
            bd.eliminarDiscografica(id);
            //du.eliminarAutor(id);
            mFrame.consultarDiscos();
            cargarDatos();
    ...
    private void eliminarButtonMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_eliminarButtonMouseClicked
        if (evt.getButton() == 1) {
            if (Lista.getSelectedRow() != -1) {
                if (addFrame != null) {
                    addFrame.quitarDiscografica(tuplaFilaDiscografica.get(Lista.convertRowIndexToModel(Lista.getSelectedRow())));
                } else {
                    modFrame.quitarDiscografica(tuplaFilaDiscografica.get(Lista.convertRowIndexToModel(Lista.getSelectedRow())));
                }
                
                eliminarDiscografica(tuplaFilaDiscografica.get(Lista.convertRowIndexToModel(Lista.getSelectedRow())));
            }
        }
    ...
    private void selButtonMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_selButtonMouseClicked
       if (evt.getButton() == 1) {
            if (Lista.getSelectedRow() != -1) {
                if (addFrame != null) {
                    addFrame.seleccionarDiscografica(tuplaFilaDiscografica.get(Lista.convertRowIndexToModel(Lista.getSelectedRow())));
                } else {
                    modFrame.seleccionarDiscografica(tuplaFilaDiscografica.get(Lista.convertRowIndexToModel(Lista.getSelectedRow())));
                }
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(DiscograficasFrameSel.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                //new DiscograficasFrameSel(null,null).setVisible(true);
            }
        });
    }
