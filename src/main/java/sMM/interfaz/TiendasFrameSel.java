 ...
 * @author Eloy
 */
public class TiendasFrameSel extends javax.swing.JFrame {
 // Variables para mover ventana desde la barra de titulo (Dragger)
    ...
    private HashMap<Integer,Integer> tuplaFilaTienda;
    ...
    public void cargarDatos() {
        ...
        if (du != null && du.getTiendas() != null) {
            int i = 0;
            for (Map.Entry<Integer,Tienda> ed : du.getTiendas().entrySet()) {
                String[] listaAtributos = new String[1];
                listaAtributos[0] = ed.getValue().getNombre();
                tuplaFilaTienda.put(i, ed.getValue().getIdentificador());
...
//                    String nacionalidad = (String) Lista.getModel().getValueAt(row, 1);
    ...
    public TiendasFrameSel(MainFrame f, AddDiscoFrame f2) {
        mFrame = f;
        addFrame = f2;
        initComponents();
        configComponentes();
    ...
    public TiendasFrameSel(MainFrame f, ModDiscoFrame f2) {
        mFrame = f;
        modFrame = f2;
        initComponents();
        configComponentes();
    ...
    private void subirTienda(String n) {
        ConexionBD bd = ConexionJDBC.getInstance();
        try {
            int id = bd.añadirTienda(new Tienda(n));
    ...
    private void modificarTienda(int id, String n) {
        ConexionBD bd = ConexionJDBC.getInstance();
        try {
            bd.modificarTienda(id, n);
            mFrame.consultarDiscos();
    ...
    private void aplicarButtonMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_aplicarButtonMouseClicked
        if (evt.getButton() == 1) {
            if (nombreField.isEditable() && du != null && !nombreField.getText().isEmpty()) { // Nuevo/modificacion disponible
                if (Lista.getSelectedRow() == -1 && !du.existeTienda(nombreField.getText())) { // Nuevo autor
                    subirTienda(nombreField.getText());
                } else if (Lista.getSelectedRow() != -1 && !du.existeAutorIgnorando(nombreField.getText(), tuplaFilaTienda.get(Lista.convertRowIndexToModel(Lista.getSelectedRow())))) { // Modificar autor
                    modificarTienda(tuplaFilaTienda.get(Lista.convertRowIndexToModel(Lista.getSelectedRow())),nombreField.getText()); 
                }
                nombreField.setEditable(false);
                nombreField.setText("");
                Lista.clearSelection();
    ...
    private void eliminarButtonMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_eliminarButtonMouseClicked
        if (evt.getButton() == 1) {
            if (Lista.getSelectedRow() != -1) {
                if (addFrame != null) {
                    addFrame.quitarTienda(tuplaFilaTienda.get(Lista.convertRowIndexToModel(Lista.getSelectedRow())));
                } else {
                    modFrame.quitarTienda(tuplaFilaTienda.get(Lista.convertRowIndexToModel(Lista.getSelectedRow())));
                }
                
                eliminarTienda(tuplaFilaTienda.get(Lista.convertRowIndexToModel(Lista.getSelectedRow())));
            }
        }
    ...
    private void selButtonMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_selButtonMouseClicked
       if (evt.getButton() == 1) {
            if (Lista.getSelectedRow() != -1) {
                if (addFrame != null) {
                    addFrame.seleccionarTienda(tuplaFilaTienda.get(Lista.convertRowIndexToModel(Lista.getSelectedRow())));
                } else {
                    modFrame.seleccionarTienda(tuplaFilaTienda.get(Lista.convertRowIndexToModel(Lista.getSelectedRow())));
                }
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(TiendasFrameSel.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                //new TiendasFrameSel(null,null).setVisible(true);
            }
        });
    }
