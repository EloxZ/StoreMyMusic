 ...
 * @author Eloy
 */
public class UbicacionesFrameSel extends javax.swing.JFrame {
 // Variables para mover ventana desde la barra de titulo (Dragger)
    Point start_drag;
    Point start_loc;
    boolean dragging;
    MainFrame mFrame;
    AddDiscoFrame addFrame;
    ModDiscoFrame modFrame;
    private HashMap<Integer,Integer> tuplaFilaUbicacion;
    ...
    public void cargarDatos() {
        ...
        tuplaFilaUbicacion = new HashMap<>();
        ...
        if (du != null && du.getUbicaciones() != null) {
            int i = 0;
            for (Map.Entry<Integer,Ubicacion> ed : du.getUbicaciones().entrySet()) {
                String[] listaAtributos = new String[1];
                listaAtributos[0] = ed.getValue().getNombre();
                tuplaFilaUbicacion.put(i, ed.getValue().getIdentificador());
...
//                    String nacionalidad = (String) Lista.getModel().getValueAt(row, 1);
    ...
    public UbicacionesFrameSel(MainFrame f, AddDiscoFrame f2) {
        mFrame = f;
        addFrame = f2;
        initComponents();
        configComponentes();
    ...
    public UbicacionesFrameSel(MainFrame f, ModDiscoFrame f2) {
        mFrame = f;
        modFrame = f2;
        initComponents();
        configComponentes();
    ...
    private void subirUbicacion(String n) {
        ConexionBD bd = ConexionJDBC.getInstance();
        try {
            int id = bd.añadirUbicacion(new Ubicacion(n));
    ...
    private void modificarUbicacion(int id, String n) {
        ConexionBD bd = ConexionJDBC.getInstance();
        try {
            bd.modificarUbicacion(id, n);
            mFrame.consultarDiscos();
    ...
    private void aplicarButtonMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_aplicarButtonMouseClicked
        if (evt.getButton() == 1) {
            if (nombreField.isEditable() && du != null && !nombreField.getText().isEmpty()) { // Nuevo/modificacion disponible
                if (Lista.getSelectedRow() == -1 && !du.existeUbicacion(nombreField.getText())) { // Nuevo autor
                    subirUbicacion(nombreField.getText());
                } else if (Lista.getSelectedRow() != -1 && !du.existeUbicacionIgnorando(nombreField.getText(), tuplaFilaUbicacion.get(Lista.convertRowIndexToModel(Lista.getSelectedRow())))) { // Modificar autor
                    modificarUbicacion(tuplaFilaUbicacion.get(Lista.convertRowIndexToModel(Lista.getSelectedRow())),nombreField.getText()); 
                }
                nombreField.setEditable(false);
                nombreField.setText("");
                Lista.clearSelection();
    ...
    private void eliminarButtonMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_eliminarButtonMouseClicked
        if (evt.getButton() == 1) {
            if (Lista.getSelectedRow() != -1) {
                if (addFrame != null) {
                    addFrame.quitarUbicacion(tuplaFilaUbicacion.get(Lista.convertRowIndexToModel(Lista.getSelectedRow())));
                } else {
                    modFrame.quitarUbicacion(tuplaFilaUbicacion.get(Lista.convertRowIndexToModel(Lista.getSelectedRow())));
                }
                
                eliminarUbicacion(tuplaFilaUbicacion.get(Lista.convertRowIndexToModel(Lista.getSelectedRow())));
            }
        }
    ...
    private void selButtonMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_selButtonMouseClicked
       if (evt.getButton() == 1) {
            if (Lista.getSelectedRow() != -1) {
                if (addFrame != null) {
                    addFrame.seleccionarUbicacion(tuplaFilaUbicacion.get(Lista.convertRowIndexToModel(Lista.getSelectedRow())));
                } else {
                    modFrame.seleccionarUbicacion(tuplaFilaUbicacion.get(Lista.convertRowIndexToModel(Lista.getSelectedRow())));
                }
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(UbicacionesFrameSel.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
              //  new UbicacionesFrameSel(null,null).setVisible(true);
            }
        });
    }
